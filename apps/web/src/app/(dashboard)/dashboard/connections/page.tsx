...
import { CheckCircle2, Chrome, Compass, KeyRound } from "lucide-react";
...
export default function ConnectionsHubPage() {
  return (
    <div className="grid gap-6">
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.24em] text-cyan-700">Connections Hub</p>
        <h1 className="mt-3 text-4xl font-semibold tracking-normal text-slate-950 sm:text-5xl">Official APIs plus direct OTA mode</h1>
      ...
      <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <div className="[&_section]:border-cyan-900/10 [&_section]:bg-white/90 [&_section]:text-slate-950">
          <PmsConnectPanel />
        </div>
          ...
          {connectionCards.map((connection) => (
            <GlassCard key={connection.name} className="p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xl font-semibold text-slate-950">{connection.name}</p>
                  <p className="mt-2 text-sm leading-6 text-slate-600">{connection.detail}</p>
                </div>
                <span className={connection.tone === "live" ? "text-emerald-700" : "text-cyan-700"}>
                  <CheckCircle2 className="size-5" />
                </span>
              </div>
              <div className="mt-5 rounded-full border border-cyan-900/10 bg-white/70 px-3 py-2 text-sm text-slate-600">
                {connection.status}
              </div>
            </GlassCard>
          ))}
      ...
      <GlassCard className="p-6">
        <PanelTitle
          eyebrow="Direct OTA automation"
          title="Add a property before connecting direct OTA mode"
          copy="The previous sample property and sample rate push were removed. Once a real property exists, this area can show property-specific OTA credentials and approved rate pushes."
        />
      </GlassCard>
    </div>
  );
...
function ExtensionInstallCard({ icon: Icon, title, href, status }: { icon: typeof Chrome; title: string; href: string; status: string }) {
  return (
    <div className="rounded-3xl border border-cyan-900/10 bg-cyan-50/80 p-5">
